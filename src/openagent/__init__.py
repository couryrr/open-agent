...
def main():
    ...
    parser.add_argument('--attach-session',
                        action='store_true', help="attach to a session")
    parser.add_argument(
        '--add-provider', action='store_true', help="add a provider")
    parser.add_argument('--list-providers',
                        action='store_true', help="list all providers")
    parser.add_argument('--remove-provider', help="remove a provider")
    parser.add_argument('--add-provider-model',
                        action='store_true', help="add a model to a provider")
    parser.add_argument('--remove-provider-model',
                        action='store_true', help="remove a model from a provider")

    data_dir = user_data_dir("openagent")
    ...
    with open(os.path.join(data_dir, "data.json"), "r") as file:
        ...
        if args.create_session:
            agent.create_session(OpenAgentProvider(
                name="ollama"), name=f"{args.create_session}")
        ...
        if args.add_provider:
            ...
            agent.add_provider(OpenAgentProvider(
                name=name, url=url, port=port))
